...
function parsirajOpis(opis) {
  // Format: "1x Buket | Primalac: Ime | Adresa: ... | Grad: ..."
  ...
  for (const dio of dijelovi) {
    if (dio.startsWith('Primalac: ')) {
      result.primalac = dio.replace('Primalac: ', '');
    } else if (dio.startsWith('Adresa: ')) {
      result.adresa = dio.replace('Adresa: ', '');
    } else if (dio.startsWith('Grad: ')) {
      result.grad = dio.replace('Grad: ', '');
    } else {
      result.stavke = dio;
    }
  }
